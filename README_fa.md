...
<p align="center">
  <img src="https://img.shields.io/badge/version-5.1.4-2ca5e0" alt="نسخه ۵٫۱٫۴">
...
\* چک‌لیست *تعاملی* تنها استثناست: تلگرام اجازه می‌دهد بات فقط از طرف یک حساب بیزینس متصل و در چت
خصوصی چک‌لیست بفرستد. اگر نداشته باشی، برنامه فهرست کارها را به‌شکل ☑ / ☐ داخل خود پیام می‌فرستد —
بخش [چک‌لیست‌ها](#-چکلیستها) را ببین.
...
چک‌لیست بخشی از پیام غنی نیست؛ متد جداگانهٔ خودش را دارد — `sendChecklist` — و تلگرام آن را محدود
کرده به **باتی که از طرف یک حساب بیزینس متصل و در چت خصوصی عمل می‌کند**. یک توکن بات معمولی، هر
شکلی هم که پیلود داشته باشد، نمی‌تواند چک‌لیست بفرستد.

پس برنامه هر دو مسیر را دارد:

| تنظیمات ▸ شناسهٔ اتصال بیزینس | چت | چه ارسال می‌شود |
|---|---|---|
| پر | خصوصی (شناسهٔ عددی) | چک‌لیست تعاملی واقعی با `sendChecklist` — برنامه عنوانش را می‌پرسد |
| پر | کانال یا گروه | فهرست ☑ / ☐ داخل پیام، همراه توضیح در نوتیفیکیشن |
| خالی | هر چیزی | فهرست ☑ / ☐ داخل پیام، همراه توضیح در نوتیفیکیشن |

دو محدودیت از خود API می‌آید: **۳۰ کار** و **۱۰۰ کاراکتر** برای هر کار. و کار را نمی‌شود از قبل
تیک‌خورده فرستاد — تیک‌زدن بعداً با `markChecklistTasksAsDone` انجام می‌شود — پس کارهایی که در
ادیتور تیک زده‌ای بدون تیک می‌رسند و برنامه قبل از ارسال این را می‌گوید.
...
سریالایزر HTML  src/shared/html-serializer.js
   ...
   rich_message: { html, is_rtl?, skip_entity_detection? }
        ...
        └──► sendChecklist          چک‌لیست، فقط با اتصال بیزینس
```
...
| چک‌لیست | جدا می‌شود و با `sendChecklist` می‌رود — یا به‌شکل ☑ / ☐ داخل پیام |
...
npm run build         # نصب‌کنندهٔ ویندوز → dist/TelegramFreeRich-Setup-5.1.4.exe
